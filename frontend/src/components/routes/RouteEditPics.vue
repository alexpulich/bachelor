<template>
  <div class="shadow bg-white p-4 border rounded" v-if="route">
    <h2>
      ...
      Редактирование фотографий трассы "{{ route.name }}"
    </h2>
    <div class="row mb-4">
  ...
  export default {
    ...
    methods: {
      deleteImage (imageIndex) {
        this.$store.dispatch('routes/deleteImage', Object.assign({}, this.pictures[imageIndex]))
      },
      uploadImage () {
        this.$store.dispatch('routes/uploadImage', Object.assign({}, this.form))
      },
      processFile (event) {
